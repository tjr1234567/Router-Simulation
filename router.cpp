...
void load() {
    std::cout << "Please enter the absolute path to the text file you want to use:";
    std::string fname;
    std::string type;
    do {
        std::cin >> fname;
        ...
        if (num < 4) {
            std::cout << "Only TXT type files are accepted!" << std::endl << "Please enter again:";
            continue;
        }
        type = fname.substr(num - 4);
        if (type.compare(".txt") != 0) {
            std::cout << "Only TXT type files are accepted!" << std::endl << "Please enter again:";
        }

    } while (type.compare(".txt") != 0);
    std::fstream fib(fname);
    std::string buf;
    int port4;
    char n;
    char mask1;
    ...
    while (fib >> buf)
    {
        ...
        m->masklong = 0;
        for (int j = 0; j < len; j++) {
            m->masklong = m->masklong * 2 + 1;
        }
        m->masklong = m->masklong << (32 - len);
        for (int j = 0; j < 4; j++) {
            mas[j] = m->masklong; m->masklong = m->masklong >> 8;
        }
        m->masklong = 0;
        for (int j = 0; j < 3; j++) {
            m->masklong = m->masklong + mas[j]; m->masklong = m->masklong << 8;
        }
        m->masklong = m->masklong + mas[3];
        fib.get(); fib.get();//读取出文件中的两个空格
        fib.get(n);
        port4 = n - '0';
        m->port = port4;
...
int classfy(link* temp) {
    ...
    for (i = 0; i < num4; i++) {
        ...
        /*b0 = b0 << (32 - ip[i]->mask);
        b2 = b2 << (32 - ip[i]->mask);*///子网掩码移位匹配
        b0 = b0 & ip[i]->masklong;
        b2 = b2 & ip[i]->masklong;  //子网掩码按位与运算进行匹配
